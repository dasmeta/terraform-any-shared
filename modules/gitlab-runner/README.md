...
| <a name="input_build_job_limits"></a> [build\_job\_limits](#input\_build\_job\_limits) | The CPU allocation given to and the requested for build containers | `map(any)` | <pre>{<br>  "cpu": "2",<br>  "memory": "3Gi"<br>}</pre> | no |
...
| <a name="input_cache"></a> [cache](#input\_cache) | Describes the properties of the cache. | <pre>object({<br>    type   = string<br>    path   = string<br>    shared = bool<br>    size   = optional(string, "50Gi")<br>  })</pre> | <pre>{<br>  "path": "",<br>  "shared": false,<br>  "size": "50Gi",<br>  "type": "local"<br>}</pre> | no |
...
| <a name="input_docker_cert"></a> [docker\_cert](#input\_docker\_cert) | Mounts docker certificate configuration to the ConfigMap. | `map` | <pre>{<br>  "enabled": false,<br>  "medium": "Memory",<br>  "mount_path": "/certs/client",<br>  "name": "docker-certs"<br>}</pre> | no |
...
| <a name="input_runner_image_registry"></a> [runner\_image\_registry](#input\_runner\_image\_registry) | The docker gitlab runner version. https://hub.docker.com/r/gitlab/gitlab-runner/tags/ | `string` | `"registry.gitlab.com"` | no |
